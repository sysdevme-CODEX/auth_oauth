...
class OAuthController(http.Controller):
    ...
    def signin(self, **kw):
        ...
        try:
            ...
            credential = {'type': 'oauth_token', 'login': login, 'token': key}
            auth_info = request.session.authenticate(request.env, credential)
            uid = auth_info['uid']
            if not uid:
                _logger.error("OAuth authenticate failed: db=%s login=%s", dbname, login)
                return request.redirect('/web/login?oauth_error=2', 303)

            resp = request.redirect(_get_login_redirect_url(uid, url), 303)
            ...
            _logger.exception("AttributeError during OAuth signin (NOT auth_signup missing). db=%s",dbname)
